...
def generate_pdf(info, fi, flat_v, profile, grade_df,
    ...
    # ── HERO ZONE — nom + score + verdict ─────────────────────────
    # Nom de la course
    pdf.set_xy(15, 30)
    pdf.set_font("Helvetica", "B", 18)
    pdf.set_text_color(*C_WHITE)
    pdf.cell(120, 10, clean(info.get('name', 'Course').upper()), border=0)

    # Score — grand chiffre à droite
    if _score is not None:
        pdf.set_xy(145, 28)
        pdf.set_font("Helvetica", "B", 36)
        pdf.set_text_color(*_score_color)
        pdf.cell(30, 14, clean(str(_score)), border=0, align="C")
        pdf.set_xy(145, 41)
        pdf.set_font("Courier", "", 5)
        pdf.set_text_color(*C_DIM)
        pdf.cell(30, 4, clean("/ 100"), border=0, align="C")

    # Verdict code — micro-label au-dessus du label principal
    pdf.set_xy(15, 40)
    pdf.set_font("Courier", "", 6)
    pdf.set_text_color(*C_DIM)
    pdf.cell(120, 4, clean(f"VERDICT · {_vcode}"), border=0, ln=True)

    # Verdict label — 16pt pour hiérarchie visuelle comparable au score
    pdf.set_xy(15, 44)
    ...
    if _vsub:
        ...
        pdf.multi_cell(130, 4, clean(_vsub))
    ...
    cap_y = int(pdf.get_y()) + 3  # dynamique — évite collision si _vsub long
    ...
    if not _insuf and _pattern:
        ...
        if _pattern in ('COLLAPSE A', 'COLLAPSE B', 'COLLAPSE'):
            ...
            pdf.set_text_color(*C_RED)
            pdf.cell(0, 6, clean("EFFONDREMENT CARDIAQUE DETECTE"), ln=True)
            ...
            if d_pct is not None:
                ...
                pdf.cell(0, 5, clean(f"Pattern : {_pattern}"), ln=True)
                ...
                if ef1 and ef2:
                    ...
                    pdf.set_font("Helvetica", "", 7)
                    pdf.set_text_color(*C_DIM)
                    pdf.cell(85, 4, clean(f"EF 1ere moitie (plat) : {ef1:.3f}"), border=0)
                    pdf.set_text_color(*d_col)
                    pdf.cell(0, 4, clean(f"EF 2eme moitie (plat) : {ef2:.3f}  |  Derive (plat) : {d_pct:.1f}%"), ln=True)
    ...
    if perf and _score is not None:
        ...
        sub_data = [
            ("GAP Q4/Q1",          _score_gap, int(weights.get('gap', 0) * 100)),
    ...
    for rec in recs[:2]:
        ...
        pdf.rect(15, y_start, 2, 20, 'F')

        pdf.set_font("Courier", "", 5)
        pdf.set_text_color(*col)
        pdf.set_xy(19, y_start + 1)
        pdf.cell(0, 3, clean(label), ln=True)
        ...
        pdf.set_text_color(*C_MID)
        pdf.set_x(19)
        pdf.multi_cell(170, 4, clean(rec.get('body', '')))
        pdf.ln(3)
