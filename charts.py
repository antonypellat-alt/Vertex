...
def generate_pdf(info, fi, flat_v, profile, grade_df,
    ...
    if splits:
        ...
        # En-têtes
        col_w = [10, 20, 20, 14, 14]
        if has_hr:  col_w.append(14)
        if has_cad: col_w.append(14)
    ...
    if _vsub:
        pdf.set_font("Helvetica", "", 6)
        pdf.set_text_color(*C_MID)
        pdf.multi_cell(0, 4, clean(_vsub), align="C")
    pdf.ln(2)
        ...
        if _pattern in ('COLLAPSE A', 'COLLAPSE B', 'COLLAPSE'):
            ...
            if d_pct is not None:
                ...
                if ef1 and ef2:
                    ...
                    pdf.set_font("Helvetica", "", 7)
                    pdf.set_text_color(*C_DIM)
                    pdf.cell(85, 4, clean(f"EF 1ere moitie : {ef1:.3f}"), border=0)
                    pdf.set_text_color(*d_col)
                    pdf.cell(0, 4, clean(f"EF 2eme moitie : {ef2:.3f}  |  Derive : {d_pct:.1f}%"), ln=True)
