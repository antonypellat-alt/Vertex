...
def generate_pdf(info, fi, flat_v, profile, grade_df,
    ...
    def bg():
        pass  # géré par VertexPDF.header()
    ...
    pdf.set_font("Helvetica", "", 8)
    pdf.set_text_color(42, 64, 80)
    pdf.cell(0, 5, clean("PERFORMANCE INTELLIGENCE  |  RACE ANALYSIS v3.5"), ln=True, align="C")
    pdf.set_text_color(100, 130, 150)
    pdf.cell(0, 5, clean(f"{info['name']}  ·  {datetime.now().strftime('%d/%m/%Y')}"), ln=True, align="C")
    pdf.ln(4)

    # ── VERDICT + SCORE — en tête du PDF (N1 Sprint 3) ──────────
    # Fix B1/B2 : verdict lu depuis compute_verdict() via parametre,
    # plus de recalcul local — coherence garantie avec l'UI.
    if perf:
        _dr     = fi.get('decay_ratio', float('nan'))
        _dp_nan = fi.get('decay_pct', float('nan'))
        ...
        if verdict:
            ...
            _vcolor = _hex_to_rgb.get(verdict['color'], (65, 200, 232))
            _vlabel = verdict['label']
            _vsub   = verdict['sub']
        else:
            _vcolor = (42, 64, 80)
        ...
        if _vsub:
            ...
            pdf.cell(0, 5, clean(_vsub), ln=True)
        pdf.ln(2)
        ...
        kpi("  Efficacite cardiaque :", ef_str)
        var_label = f"{perf['score_var']} / 100  (poids {int(perf['weights']['var']*100)}%)"
        if perf.get('var_neutralized'):
            var_label += "  [neutralise - D+ eleve]"
        kpi("  Regularite :", var_label)
        ...
        # P5 — Disclaimer Minetti 2002
        pdf.set_font("Courier", "", 6)
        pdf.set_text_color(30, 50, 60)
        pdf.cell(0, 5, clean("Score base sur modele GAP Minetti (2002) -- EF calculee sur segments plats uniquement"), ln=True)
        pdf.ln(2)
    ...
    if info.get('hr_mean'):
        ...
        kpi("FCmax reelle (saisie) :", f"{fcmax} bpm")
    ...
    dr = fi['decay_ratio']
    dp = fi['decay_pct']
    kpi("Ratio Q4/Q1 (GAP) :", f"{dr:.3f}" if not _isnan(dr) else "N/A")
    kpi("Perte de vitesse :", f"{dp:.1f}%" if not _isnan(dp) else "N/A")
    for q, v in fi['quartiles'].items():
        val = f"{v:.3f} m/s  ({v_to_pace(v)} /km)" if not _isnan(v) else "N/A"
        pdf.set_font("Helvetica", "", 8)
        pdf.set_text_color(42, 64, 80)
    ...
    pdf.cell(0, 4,
        clean(f"VERTEX v3.5 -- GAP : Minetti et al. (2002) J Exp Biol -- FCmax: {fcmax} bpm -- {datetime.now().strftime('%d/%m/%Y')}"),
        ln=True, align="C")
