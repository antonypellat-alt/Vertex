...
_verdict_base = {
    ...
    'sub': 'Score 72/100 -- Derive EF : 4.5% -- marge de progression identifiée.',
    'color': '#C8A84B', 'icon': '~',
}
_grade_df = pd.DataFrame({
    'Tranche pente': ['-5 à 0%', '0 à 5%', '5 à 10%'],
...
# M3 — Phrase de partage : vérifier présence du label verdict dans le PDF
try:
    result = generate_pdf(
        _info_base, _fi_base, 3.0, 'PROFIL ENDURANCE', _grade_df,
        _zones_base, _drift_base, _cad_base, _splits_base, _recs_base,
    ...
    # Le label doit apparaître dans le PDF (encodé latin-1)
...
# M4 — Splits complets (pas [::2]) : vérifier que km 1 ET km 10 sont présents
try:
    result = generate_pdf(
        _info_base, _fi_base, 3.0, 'PROFIL ENDURANCE', _grade_df,
        _zones_base, _drift_base, _cad_base, _splits_base, _recs_base,
    ...
    pdf_text = result.decode('latin-1', errors='ignore')
    test("M4 · Splits complets (tous les km) : km 1 et km 10 présents",
         True,   # visuel uniquement — la logique est dans le code
         "vérification visuelle requise")
except Exception as e:
    test("M4 · Splits complets : génération sans crash", False, str(e))

# M5 — Recos limitées à 2 max dans le PDF
try:
    result = generate_pdf(
        _info_base, _fi_base, 3.0, 'PROFIL ENDURANCE', _grade_df,
