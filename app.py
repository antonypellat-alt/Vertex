...
def render_dashboard(gpx_bytes: bytes, filename: str):
    ...
    fcmax        = int(st.session_state.get('fcmax_confirmed', int(info['hr_max']) if info.get('hr_max') is not None else 190))
    ...
    if info['has_hr']:
        ...
        cols_kpi[5].metric("FC MAX RÉELLE", f"{fcmax} bpm", delta=f"pic : {int(info['hr_max'])} bpm")
    ...
    if info['has_hr']:
        ...
        if hr_grade is not None and len(hr_grade) > 3:
            ...
            with hz2:
                ...
                # Bandeau COLLAPSE au-dessus du graphe EF
                ...
                # Résumé dérive EF — masqué si COLLAPSE
