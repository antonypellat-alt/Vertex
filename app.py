...
def render_landing():
    ...
    features = [
        ...
        ("ZONES FC",     "Où tu as couru — et combien ça t'a coûté",          "Distribution Z1→Z5 · Découplage cardiaque · EF"),
...
def render_dashboard(gpx_bytes: bytes, filename: str):
    ...
    drift    = cardiac_drift(df,
                   ...
                   dp_per_km=_dp_per_km) if info['has_hr'] else {
    ...
    perf     = compute_performance_score(fi, drift, dp_per_km=_dp_per_km)
    ...
    for _wlevel, _wtitle, _wmsg in _warnings:
        ...
        # UX-3 : 2 niveaux structurels — crit = pleine attention, warn/info = informatif
        if _wlevel == "crit":
            _pad    = "14px 18px"
            _bwidth = "4px"
            _title_w = "font-weight:700;"
        elif _wlevel == "warn":
            _pad    = "10px 16px"
            _bwidth = "3px"
            _title_w = "font-weight:500;"
        else:  # info
            _pad    = "8px 16px"
            _bwidth = "2px"
            _title_w = "font-weight:400;"
        st.markdown(f"""
    ...
    # ── Bloc VERDICT — UX-1 : premier élément visible après warnings ──
    _v      = verdict
    _vcode  = _v['code']
    _vlabel = _v['label']
    _vsub   = _v['sub']
    ...
    if _score >= 80:
        ...
        _score_interp2 = "Ralentissement GAP ou dérive EF détectés sur la 2e moitié."
    else:
        _score_interp1 = "Effort mal dosé — faiblesse significative détectée."
        _score_interp2 = "Chute d'allure normalisée et/ou découplage cardiaque marqué."
    ...
    # Dérive EF
    if _s_ef is None:
        _ef_interp = "FC absente — EF non calculable."
    elif _s_ef >= 80:
    ...
    # ── Carte score global — toujours visible ────────────────────
    st.markdown(f"""
    <div style="background:#0D1520;border:1px solid #152030;
                border-top:2px solid {_score_color};padding:20px 24px;margin-bottom:8px;">
        <div style="font-family:'DM Mono',monospace;font-size:0.6rem;
        ...
        <div style="font-family:'DM Mono',monospace;font-size:0.6rem;color:#3A5060;margin-top:6px;">
            {"⚠ " + _p_reason if _partial else "Score complet · 3 composantes"}
        </div>
        <div style="font-family:'DM Mono',monospace;font-size:0.6rem;
                    color:{_score_color};margin-top:6px;line-height:1.5;">
            {_score_interp1}<br>
    ...
    with st.expander("▶  DÉTAIL DU SCORE · GAP / EF / RÉGULARITÉ", expanded=False):
        st.markdown(f"""
    ...
    if info['has_cad']:
        ...
        c3.metric("ÉCART GAP Q4/Q1", f"{dp:.1f}%" if not math.isnan(dp) else "N/A")
        ...
        if info['has_hr'] and drift:
            ...
            if not _insufficient:
                if _pattern == 'COLLAPSE':
                    ...
                    c4.metric("DÉRIVE CARDIO", f"{_drift_pct:.1f}%")
    ...
    if info['has_hr'] and drift:
        ...
        if _di:
            ...
            _dlabel = 'DÉRIVE CARDIAQUE'
            _dcolor = '#C8A84B'
            _dsub   = 'L\'efficacité cardiaque a progressivement baissé au fil de la course — signal attendu sur cette distance. Pour réduire cette dérive : travaille le volume en Z2 sur tes prochaines semaines.'
            _dmetrics_html = f"""
            <div style="display:flex;flex-wrap:wrap;gap:24px;margin-top:10px;">
                <div>
                    <div style="font-family:'DM Mono',monospace;font-size:0.55rem;color:#3A5060;letter-spacing:0.18em;">FC Q1</div>
                ...
                <div>
                    <div style="font-family:'DM Mono',monospace;font-size:0.55rem;color:#3A5060;letter-spacing:0.18em;">BAISSE CARDIO</div>
