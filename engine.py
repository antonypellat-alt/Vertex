...
def cardiac_drift(df: pd.DataFrame) -> dict:
    ...
    COLLAPSE A : FC chute > 10% OU slope < -3.0 bpm/h (segments plats) — Sprint 4
    ...
    # COLLAPSE A : FC chute > 10% OU slope < -3.0 bpm/h (CDC Elena v1.1 — Sprint 4)
    # COLLAPSE B : FC chute > 20% quelle que soit la pente (segments plats uniquement)
    # DRIFT      : EF dégrade < -2% sans COLLAPSE
    # STABLE     : tout le reste
    collapse_a = (fc_delta_pct is not None and (fc_delta_pct < -10 or fc_slope_bph < -3.0))
