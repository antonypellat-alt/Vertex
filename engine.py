...
def generate_coach_recommendations(
    ...
    # ── R-C : DRIFT ─────────────────────────────────────────────
        ...
        _body_drift = {
            'COURT': (
                f"L'efficacité cardiaque a baissé de {abs(drift_pct):.1f}% sur la 2ème moitié. "
                "Renforcer l'endurance aérobie : 2 sorties/semaine en Z2 strict (60-70% FCmax), 45-60 min."
            ),
            'LONG': (
                f"L'efficacité cardiaque a baissé de {abs(drift_pct):.1f}% après la mi-course. "
                "Allonger les sorties longues : 1×/semaine ≥2h en Z2 pour repousser le seuil de dérive."
            ),
            'ULTRA': (
                f"L'efficacité cardiaque a baissé de {abs(drift_pct):.1f}% — normal sur cette durée, mais optimisable. "
...
def compute_verdict(fi: dict, drift: dict, perf: dict) -> dict:
    ...
    if score < 75:
        return {
            ...
            'sub':   f"Score {score}/100 — bonne course, mais l'allure n'a pas été régulière sur la durée. Travaille la régularité d'allure sur tes prochaines sorties longues.",
            'color': '#C8A84B',
            'icon':  '~',
        }
